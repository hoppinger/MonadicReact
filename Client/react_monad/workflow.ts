...
export let simple_workflow = function<S,M>(workflow_name:string, steps:Immutable.Map<S, (_:WorkflowData<S,M>) => C<WorkflowData<S,M>>>, initial_model:M, initial_step:S) : C<M> {
  return repeat<WorkflowData<S,M>>(cd =>
    steps.has(cd.step) ?
      steps.get(cd.step)(cd)
    :
      unit<WorkflowData<S,M>>(cd).filter(_ => false)
  )({ model: initial_model, step:initial_step }).map<M>(c => c.model)
}
